...
for part in score.parts:
    ...
    for measure in part.getElementsByClass('Measure'):
        ...
        if (measure_number == len(chord_changes)+1):
            break
        ...
        for i, (start_offset, end_offset) in enumerate(partitions):
            ...
            if count == 0:
                if i == 0:
                    chord_changes[measure_number-1][i] = 0.9*chord_changes[measure_number-2][3]
                else:   
                    chord_changes[measure_number-1][i] = 0.9*chord_changes[measure_number-1][i-1]
            else:
