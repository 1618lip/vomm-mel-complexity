...
vector<string> splitChords(const string& measure, bool withspace) {
    ...
    string rgx;
    if (withspace) {
        rgx = R"(\s|([A-G](#|b)?(-|j|o|\+|sus|add)?([1-9]*)?(\/[A-G](#|b)?)?))";
    }
    else {
        rgx = R"(([A-G](#|b)?(-|j|o|\+|sus|add)?([1-9]*)?((#|b)[1-9]*)?(\/[A-G](#|b)?)?))";
    }
...
string changeChordNaming(string chord) {
    string newName = "";
    for (int i = 0; i < chord.length(); i++) {
        if (changeNaming.count(chord[i]) != 0) {
            newName = newName + changeNaming[chord[i]];
        }
        else {
            newName = newName + chord[i];
        }
    }
    return newName;
}
